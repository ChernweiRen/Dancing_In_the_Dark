...
class UpConvBlock(nn.Module):
    def __init__(self, in_features, out_channels, up_scale):
    ...
    def compute_out_features(self, idx, up_scale):
        return self.out_channel if idx == up_scale - 1 else self.constant_features
...
class DexiNed(nn.Module):
    ...
    def __init__(self, inchannel=3, block_size=2):
        super(DexiNed, self).__init__()
        self.in_channels = inchannel
        self.block_size = block_size
        self.out_channels = 1 * self.block_size * self.block_size

        self.block_1 = DoubleConvBlock(self.in_channels, 32, 64, stride=2,)
        ...
        # USNet
        self.up_block_1 = UpConvBlock(64, 1*self.out_channels, 1)
        self.up_block_2 = UpConvBlock(128, 1*self.out_channels, 1)
        self.up_block_3 = UpConvBlock(256, 1*self.out_channels, 2)
        self.up_block_4 = UpConvBlock(512, 1*self.out_channels, 3)
        self.up_block_5 = UpConvBlock(512, 1*self.out_channels, 4)
        self.up_block_6 = UpConvBlock(256, 1*self.out_channels, 4)
        self.block_cat = SingleConvBlock(6*self.out_channels, 1*self.out_channels, stride=1, use_bs=False) # hed fusion method
    ...
    def forward(self, x):
        ...
        block_cat = torch.cat([out_1, out_2, out_3, out_4, out_5, out_6], dim=1)  # Bx6xHxW
        block_cat = self.block_cat(block_cat)  # Bx1xHxW

        # return results
        out_1 = pixel_shuffle(out_1, upscale_factor=self.block_size, depth_first=True)
        out_2 = pixel_shuffle(out_2, upscale_factor=self.block_size, depth_first=True)
        out_3 = pixel_shuffle(out_3, upscale_factor=self.block_size, depth_first=True)
        out_4 = pixel_shuffle(out_4, upscale_factor=self.block_size, depth_first=True)
        out_5 = pixel_shuffle(out_5, upscale_factor=self.block_size, depth_first=True)
        out_6 = pixel_shuffle(out_6, upscale_factor=self.block_size, depth_first=True)

        out = pixel_shuffle(block_cat, upscale_factor=self.block_size, depth_first=True)

        return [out_1, out_2, out_3, out_4, out_5, out_6], out

def dexined(batch_norm=False,layer_num=5,fusion_layer=5,**kwargs):
    if batch_norm==1:
        if fusion_layer == 6:
...
if __name__ == '__main__':
    ...
    # device = "cpu"
    input = torch.rand(batch_size, 4, img_height, img_width).to(device)
    # target = torch.rand(batch_size, 1, img_height, img_width).to(device)
    print(f"input shape: {input.shape}")
    model = DexiNed(inchannel=4).to(device)
    output = model(input)
